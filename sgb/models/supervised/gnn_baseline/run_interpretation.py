"""V2: Node-level subgraph-ablation interpretation for GCN/GAT/SAGE.

Reuses gnn_baseline FT pipeline; replaces feature-mask compute_fidelity
with subgraph-mask compute_node_fidelity (GraphFramEx-style).

Outputs [INTERP_NODE_V2_RAW] one line per (split, seed, explainer, topk),
plus [INTERP_NODE_V2_AGG] aggregates per (explainer, topk).
"""
import copy
...
flags.DEFINE_integer('patience', 100, 'Early stop patience.')
flags.DEFINE_float('lr', 1e-3, 'Learning rate.')
flags.DEFINE_float('weight_decay', 1e-4, 'Weight decay.')
flags.DEFINE_float('dropout', 0.2, 'Dropout.')
flags.DEFINE_integer('max_test_nodes', 100, 'Max test nodes per (split, seed) for v2 interp.')
flags.DEFINE_integer('K_hop', 2, 'Receptive-field hops.')
flags.DEFINE_integer('n_seeds', 3, 'Number of run seeds.')
flags.DEFINE_integer('n_splits', 1, 'Number of splits to use (1 enough for v2; uses split 0).')
flags.DEFINE_string('explainers', 'grad,random', 'Comma-separated explainers to run.')
flags.DEFINE_string('topk_list', '0.05,0.10,0.20,0.50', 'Comma sparsity values.')
flags.DEFINE_bool('debug', False, 'smoke: 1 split 1 run, 5 test nodes.')
...
def main(argv):
    ...
    print(f"[{tag} V2-INTERP] device={device} dataset={FLAGS.dataset}")
    ...
    print(f"[{tag} V2-INTERP] N={N} d={input_size} C={num_classes} E={data.edge_index.size(1)}")

    splits = _build_splits(data)
    n_splits = 1 if FLAGS.debug else FLAGS.n_splits
    ...
    # Aggregate across (split, seed) -> per (explainer, topk) collect Fid+/Fid-/char
    agg = {(e, tk): {"fp": [], "fm": [], "ch": [], "n_eval": []} for e in explainers for tk in topk_list}

    for split_idx in range(min(n_splits, len(splits))):
        tm, vm, tsm = splits[split_idx]
        ...
        for rs in RUN_SEEDS[:n_seeds]:
            ...
            print(f"[{tag} V2-INTERP] split={split_idx} seed={rs} clean_acc={acc:.2f}")

            # forward_fn substitutes data.x and data.edge_index
            ...
            rng_local = np.random.RandomState(rs)
            sample = rng_local.choice(test_idx, size=min(max_test, len(test_idx)), replace=False)
            ...
            for explainer in explainers:
                ...
                for tk in topk_list:
                    ...
                    print(f"[INTERP_NODE_V2_RAW] method={tag} dataset={FLAGS.dataset} "
                          f"split={split_idx} seed={rs} explainer={explainer} topk={tk} "
    ...
    for (explainer, tk), d in agg.items():
        ...
        print(f"[INTERP_NODE_V2_AGG] method={tag} dataset={FLAGS.dataset} "

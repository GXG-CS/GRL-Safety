"""GNN baseline (GCN/GAT/SAGE) FT + structural (degree-based) fairness.

Protocol:
  - Standard FT on clean train set (no perturbation).
  - On test set: compute per-node predictions, assign to head/tail by degree.
  - acc_head / acc_tail / acc_gap; f1_head / f1_tail / f1_gap (Kang 2020, Liu 2022).
  - 5 split × 5 seed = 25 runs per method-dataset (except citeseer: 5 runs).
Emits [STRUCT_RAW] / [STRUCT_AGG] lines.
"""
...
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'tolokers', 'Dataset name.')
flags.DEFINE_string('model', 'gcn', 'GNN baseline: gcn, gat, sage.')
...
flags.DEFINE_float('q', 0.2, 'Quantile for head/tail split.')
...
def main(argv):
    ...
    print(f"[{tag} FT-STRUCT] Using {device}, dataset={FLAGS.dataset}")
    ...
    # Compute in-degree per node (undirected)
    ei = data.edge_index
    degree = torch.bincount(ei[0], minlength=N).cpu().numpy()
    print(f"[{tag} FT-STRUCT] N={N}, d={input_size}, C={num_classes}, "
    ...
    for split_idx, (tm, vm, tsm) in enumerate(splits):
        ...
        for rs in run_seeds:
            ...
            struct = compute_structural_fairness(y_np, preds, degree,
                                                 test_mask=tsm_np, q=FLAGS.q)
            print(f"[STRUCT_RAW] method={tag} dataset={FLAGS.dataset} split={split_idx} "
                  f"seed={rs} acc={acc:.4f} "
                  ...
                  f"n_head={struct['n_head']} n_tail={struct['n_tail']} q={FLAGS.q}")
    ...
    a, sa = _agg('acc')
    ah, sah = _agg('acc_head'); at, sat = _agg('acc_tail'); ag, sag = _agg('acc_gap')
    fh, sfh = _agg('f1_head'); ft, sft = _agg('f1_tail'); fg, sfg = _agg('f1_gap')
    print(f'[STRUCT_AGG] method={tag} dataset={FLAGS.dataset} n_runs={len(results)} '
          f'acc="{a:.2f} ± {sa:.2f}" '

"""GNN baseline OOD eval — TIME SHIFT variant.

Protocol: covariate shift along publication year. Oldest 60% -> train pool,
next 5% -> ood_val, newest 35% -> ood_test. Only arxiv (has node_year).
"""
...
# Only arxiv has node_year in our TAG pipeline (confirmed by inspection)
NODE_DATASETS = {"arxiv"}
...
flags.DEFINE_integer("train_max_year", 0, "If >0, use aggressive year cutoff (train<=this).")
flags.DEFINE_integer("ood_min_year", 0, "If >0, use aggressive year cutoff (ood>=this).")
...
def main(argv):
    ...
    # Load raw data again to get node_year (load_tag may strip it)
    ...
    year = data.node_year.to(data.x.device)

    _tmy = FLAGS.train_max_year if FLAGS.train_max_year > 0 else None
    _omy = FLAGS.ood_min_year if FLAGS.ood_min_year > 0 else None

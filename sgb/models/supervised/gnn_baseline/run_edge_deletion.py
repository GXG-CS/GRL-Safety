"""GNN baseline (GCN/GAT/SAGE) from-scratch FT + edge-deletion corruption eval.

Mirrors BGRL's run_edge_deletion.py protocol with random-init encoder.
"""
...
def _aggregate_and_print(tag, all_results, metric_name="acc"):
    print(f"\n=== {tag} FT Edge Deletion Results ===")
    grouped_acc = collections.defaultdict(list)
    grouped_f1 = collections.defaultdict(list)
    for r in all_results:
        grouped_acc[r["sev"]].append(r[metric_name] if metric_name in r else r.get("auc", r.get("acc")))
    ...
    print(f"[ED_AGG] method={tag} dataset={FLAGS.dataset} "
...
def main(argv):
    ...
    tag = method_tag()
    print(f"[{tag} FT-ED] Using {device}")
    ...
    print(f"[{tag} FT-ED] {FLAGS.dataset}, N={data.num_nodes}, E={data.edge_index.size(1)}, C={num_classes}")
    ...
    for split_idx, split in enumerate(splits):
        ...
        print(f"[ED_RAW] method={tag} dataset={FLAGS.dataset} "
...
def _run_link_ft_ed(device):
    ...
    print(f"[{tag} FT-ED Link] {FLAGS.dataset}, N={graph.num_nodes}, E={graph.edge_index.size(1)}, d={input_size}")
    ...
    for split_idx in range(5):
        ...
        for sev_idx, p in SEVERITIES:
            ...
            print(f"[ED_RAW] method={tag} dataset={FLAGS.dataset} "
                  f"split_idx={split_idx} seed={split_idx} sev={sev_idx} p={p} "
                  f"test_auc={noise_auc:.4f} test_f1={noise_f1:.4f}")

        test_data.edge_index = clean_edge_index
...
def _run_graph_ft_ed(device):
    ...
    print(f"[{tag} FT-ED] {FLAGS.dataset}: n_graphs={n_graphs}, num_tasks={num_tasks}")
    ...
    # fixed seed=42 partition across all 5 splits, which produced
    # artificial std=0 on large multi-task graph datasets (chempcba).
    ...
    for split_idx in range(5):
        ...
        for sev_idx, p in SEVERITIES:
            ...
            dropped_loader = PyGDataLoader(dropped_test, batch_size=512, shuffle=False, num_workers=0)
            drop_auc, drop_f1 = _compute_auc(dropped_loader, model)
            all_results.append({"sev": sev_idx, "auc": drop_auc, "f1": drop_f1})
            print(f"[ED_RAW] method={tag} dataset={FLAGS.dataset} "
                  f"split_idx={split_idx} seed={split_idx} sev={sev_idx} p={p} "
                  f"test_auc={drop_auc:.4f} test_f1={drop_f1:.4f}")

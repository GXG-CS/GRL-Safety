...
def main(argv):
    ...
    print(f"[BGRL] Dataset: {FLAGS.dataset}, input_dim={input_size}, repr_dim={representation_size}")
    print(f"[BGRL] Encoder: {[input_size] + FLAGS.graph_encoder_layer}")
    print(f"[BGRL] Epochs: {FLAGS.epochs}, lr: {FLAGS.lr}, mm: {FLAGS.mm}")

    for epoch in tqdm(range(1, FLAGS.epochs + 1)):
        train(epoch-1)

    # save encoder weights (same format as GFT/GIT: direct state_dict as encoder.pt)
    torch.save(model.online_encoder.state_dict(),
               os.path.join(FLAGS.logdir, 'encoder.pt'))
    print(f"[BGRL] Saved encoder to {os.path.join(FLAGS.logdir, 'encoder.pt')}")

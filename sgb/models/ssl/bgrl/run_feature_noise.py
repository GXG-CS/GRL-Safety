"""BGRL FT + feature-noise corruption evaluation.

Fine-tunes the pretrained BGRL GCN encoder end-to-end (encoder unfrozen +
linear head), then evaluates the best-val checkpoint under 5 feature-noise
severity levels. Mirrors the FT protocol of GFT/GIT.
...
def main(argv):
    ...
    print(f"[BGRL FT-FN] Using {device}")
    ...
    # ====== Node classification (original, untouched) ======
    ...
    print(f"[BGRL FT-FN] {FLAGS.dataset}, N={data.num_nodes}, d={input_size}, C={num_classes}")
    ...
    for split_idx, split in enumerate(splits):
        ...
        print(f"[FN_RAW] method=BGRL_FT dataset={FLAGS.dataset} "
    ...
    print(f"[FN_AGG] method=BGRL_FT dataset={FLAGS.dataset} "
...
def _run_link_ft_fn(device):
    """Link prediction FT + feature noise eval."""
    ...
    print(f"[BGRL FT-FN Link] {FLAGS.dataset}, N={graph.num_nodes}, E={graph.edge_index.size(1)}, d={input_size}")
    ...
    for split_idx in range(5):
        ...
        # Each has .edge_index (message-passing edges), .edge_label_index, .edge_label
    ...
    # Aggregation
    print(f"\n=== BGRL FT Link Feature Noise Results ===")
...
def _run_graph_ft_fn(device):
    """Graph classification FT + feature noise eval."""
    ...
    print(f"[BGRL FT-FN] {FLAGS.dataset}: n_graphs={n_graphs}, num_tasks={num_tasks}")

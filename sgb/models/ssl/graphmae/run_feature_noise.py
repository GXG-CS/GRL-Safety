"""GraphMAE FT + feature-noise corruption evaluation.

Loads a joint-pretrained GraphMAE encoder, attaches a linear head,
...
Outputs [FN_RAW] and [FN_AGG] lines for aggregation.
"""
...
class FTModel(nn.Module):
    """PreModel encoder (unfrozen) + linear head for node classification."""
...
def main():
    ...
    print(f"[GraphMAE FT-FN] Using {device}")
    ...
    print(f"[GraphMAE FT-FN] {args.dataset}, N={g.num_nodes()}, C={num_classes}")
    ...
    for split_idx, split in enumerate(splits):
        ...
        # Build fresh model, load pretrained, FT on clean
        pre_model = build_joint_model(num_features=feat_clean.size(1))
        pre_model.load_state_dict(state)
        model = FTModel(pre_model, num_hidden=768, num_classes=num_classes,
                        dropout=args.dropout).to(device)

        clean_acc, clean_f1 = train_ft(model, g, feat_clean, y, train_mask, val_mask, test_mask,
        ...
        print(f"[FN_RAW] method=GraphMAE_FT dataset={args.dataset} "
        ...
        for sev_idx, sigma_rel in SEVERITIES:
            feat_noisy = apply_feature_noise(feat_clean, train_mask, sigma_rel,
    ...
    print(f"\n=== GraphMAE FT Feature Noise Results ===")
    ...
    print(f"[FN_AGG] method=GraphMAE_FT dataset={args.dataset} "
...
def _run_link_ft_fn(device, args):
    """Link prediction FT + feature noise eval for GraphMAE."""
    ...
    print(f"[GraphMAE FT-FN Link] {args.dataset}, N={num_nodes}, E={graph.edge_index.size(1)}, d={input_size}")
    ...
    for split_idx in range(5):
        ...
        # Compute feature std from training node features (for noise calibration)
        ...
        # Feature noise corruption on test data
        x_clean = test_data.x.clone()
        for sev_idx, sigma_rel in SEVERITIES:
            g = torch.Generator(device=device).manual_seed(int(split_idx * 100 + sev_idx))
            eps = torch.randn(x_clean.shape, generator=g, device=device, dtype=x_clean.dtype)
        ...
        test_data.x = x_clean  # restore

    # Aggregation
    print(f"\n=== GraphMAE FT Link Feature Noise Results ===")
...
def _run_graph_ft_fn(device, args):
    """Graph classification FT + feature noise eval for GraphMAE."""
    ...
    print(f"[GraphMAE FT-FN] {args.dataset}: n_graphs={n_graphs}, num_tasks={num_tasks}")
    ...
    for split_idx in range(5):
        ...
        all_results.append({"sev": 0, "auc": clean_auc, "f1": clean_f1})
        print(f"[FN_RAW] method=GraphMAE_FT dataset={args.dataset} "
              f"split_idx={split_idx} seed={split_idx} sev=0 sigma_rel=0.0 test_auc={clean_auc:.4f} test_f1={clean_f1:.4f}")

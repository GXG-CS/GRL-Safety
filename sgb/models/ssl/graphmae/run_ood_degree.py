"""GraphMAE FT + node-level OOD evaluation (GFM-Safety Dim 2, node only).
...
Same contract as sgb/models/ssl/bgrl/run_ood_degree.py and
sgb/models/gfm/gft/GFT/run_ood_degree.py: loads the shared split artifact produced
by `sgb.data.ood_splits`, fine-tunes on the train split, tracks best-by-id_val
and best-by-ood_val checkpoints simultaneously, and emits `[OOD_RAW]`
(main-table stream) and `[OOD_ORACLE]` (appendix-only) lines.
"""
...
# .get_covariate_shift_graph: sort descending by degree, 60% / 20% / 20%
# train / ood_val / ood_test, then random-shuffle (seeded) the train slice to
# carve id_val / id_test (each 10% of total). No disk caching.
...
def build_degree_split(
    ...
    See the GFT / BGRL copy for documentation. Output keys: `train`, `id_val`,
    `id_test`, `ood_val`, `ood_test` (1-D `torch.long` tensors) plus `meta`.
    """
...
def _train_one_run(
    g, feat, y, masks, num_classes, state, args, run_seed, device,
):
...
def main():
    ...
    for split_seed in split_seeds:
        ...
        if meta.get("degree_shift") == "not_applicable":
            print(f"[OOD_SKIP] method=GraphMAE_FT dataset={args.dataset} "
        ...
        for run_seed in run_seeds:
            ...
            print(
                f"[OOD_RAW] method=GraphMAE_FT dataset={args.dataset} "
            ...
            print(
                f"[OOD_ORACLE] method=GraphMAE_FT dataset={args.dataset} "
                f"split_seed={split_seed} run_seed={run_seed} "

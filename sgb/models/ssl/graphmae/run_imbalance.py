"""GraphMAE FT + step imbalance evaluation (node classification only)."""
...
def main():
    ...
    print(f"[GraphMAE FT-IMB] device={device}, dataset={args.dataset}, rho={args.rho}, n_reps={args.n_reps}")
    ...
    print(f"[GraphMAE FT-IMB] N={g.num_nodes()}, d={input_size}, C={num_classes}")
    ...
    for rep_idx in range(args.n_reps):
        ...
        print(f"[IMB_RAW] method=GraphMAE_FT dataset={args.dataset} rho={args.rho} "
        ...
        print(f"[IMB_PER_CLASS] method=GraphMAE_FT dataset={args.dataset} rho={args.rho} "
    ...
    print(f"\n=== GraphMAE Imbalance ({args.dataset}, rho={args.rho}, n={len(all_results)}) ===")
    print(f"  bAcc={bacc.mean():.2f} ± {bacc.std():.2f}  macroF1={f1.mean():.2f} ± {f1.std():.2f}  acc={acc.mean():.2f} ± {acc.std():.2f}")
    print(f"[IMB_AGG] method=GraphMAE_FT dataset={args.dataset} rho={args.rho} "

...
TAG_DATASETS = {"cora", "pubmed", "wikics", "arxiv"}
...
def load_tag_dataset(dataset_name):
    """Load TAG dataset and convert to DGL format."""
    ...
    # Build masks — use first split
    if hasattr(data, 'train_masks'):
        graph.ndata["train_mask"] = data.train_masks[0]
        graph.ndata["val_mask"] = data.val_masks[0]
        graph.ndata["test_mask"] = data.test_masks[0]
    elif hasattr(data, 'train_mask'):
        tm = data.train_mask
        vm = data.val_mask
        tsm = data.test_mask
        if tm.dim() == 2:  # WikiCS [N,20]
            graph.ndata["train_mask"] = tm[:, 0]
            graph.ndata["val_mask"] = vm[:, 0]
            graph.ndata["test_mask"] = tsm if tsm.dim() == 1 else tsm[:, 0]
        else:
            graph.ndata["train_mask"] = tm
            graph.ndata["val_mask"] = vm
            graph.ndata["test_mask"] = tsm
...
def load_dataset(dataset_name):
    if dataset_name in TAG_DATASETS:
        return load_tag_dataset(dataset_name)

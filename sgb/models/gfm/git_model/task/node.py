...
def eval_node(model, data, split, params):
    ...
    test_pred_np = y_pred[test_mask].argmax(-1).cpu().numpy()
    test_true_np = y[test_mask].cpu().numpy()
    test_f1 = f1_score(test_true_np, test_pred_np, average='macro') * 100.0

    return {'train': train_value, 'val': val_value, 'test': test_value, 'test_f1': test_f1, 'metric': task2metric[params['task']]}

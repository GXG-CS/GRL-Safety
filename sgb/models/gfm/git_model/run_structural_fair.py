"""GIT FT + structural (degree-based) fairness."""
...
class FTModel(nn.Module):
    ...
    def forward(self, x, edge_index):
        h = self.encoder(x, edge_index); h = self.dropout(h); return self.head(h)
...
def train_ft(model, x, edge_index, y, tm, vm, lr, wd, max_epochs, patience):
    ...
    for _ in range(1, max_epochs + 1):
        ...
        F.cross_entropy(model(x, edge_index)[tm], y[tm]).backward(); optim.step()
        model.eval()
        with torch.no_grad():
...
def main():
    ...
    print(f"[GIT FT-STRUCT] dataset={args.dataset}")
    ...
    print(f"[GIT FT-STRUCT] N={N} d={input_dim} C={num_classes} deg mean={degree.mean():.1f}")
    ...
    for split_idx, (tm, vm, tsm) in enumerate(splits):
        ...
        for rs in RUN_SEEDS:
            ...
            model.eval()
            with torch.no_grad():
                preds = model(data.x, data.edge_index).argmax(-1).cpu().numpy()
                y_np = y.cpu().numpy()
            ...
            print(f"[STRUCT_RAW] method=GIT dataset={args.dataset} split={split_idx} seed={rs} "
    ...
    print(f'[STRUCT_AGG] method=GIT dataset={args.dataset} n_runs={len(results)} '

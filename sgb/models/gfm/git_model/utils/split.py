...
def base_split(data, params):
    ...
    if task == 'node':
        ...
        if has_masks:
            ...
            train_masks = list(data.train_masks)
            val_masks = list(data.val_masks)
            test_masks = list(data.test_masks)
        elif has_mask:
            tm, vm, tsm = data.train_mask, data.val_mask, data.test_mask
            # WikiCS: masks are [N, num_splits] 2D tensor
            if tm.dim() == 2:
                train_masks = [tm[:, i] for i in range(tm.size(1))]
                val_masks = [vm[:, i] for i in range(vm.size(1))]
                test_masks = [tsm[:, i] if tsm.dim() == 2 else tsm for i in range(tm.size(1))]
            else:
                train_masks = [tm] * params['repeat']
                val_masks = [vm] * params['repeat']
                test_masks = [tsm] * params['repeat']
        else:
            raise ValueError("Data does not have masks")

        # Normalize to params['repeat'] splits
        r = params['repeat']
        n = len(train_masks)
        if n < r:
            train_masks = (train_masks * ((r // n) + 1))[:r]
            val_masks = (val_masks * ((r // n) + 1))[:r]
            test_masks = (test_masks * ((r // n) + 1))[:r]
        elif n > r:
            train_masks = train_masks[:r]
            val_masks = val_masks[:r]
            test_masks = test_masks[:r]
        ...
        # KG datasets: construct edge masks from split indices if no masks present
        has_mask = hasattr(data, 'train_mask') & hasattr(data, 'val_mask') & hasattr(data, 'test_mask')
        has_masks = hasattr(data, 'train_masks') & hasattr(data, 'val_masks') & hasattr(data, 'test_masks')
        has_idx = hasattr(data, 'train_idx') & hasattr(data, 'val_idx') & hasattr(data, 'test_idx')
        if not has_mask and not has_masks and hasattr(data, 'edge_types'):
            # Read split sizes from raw KG files
            _kg_raw = osp.join(osp.dirname(__file__), "..", "..", "..", "data", "dataset", "data", "KG", params['dataset'])
            n_edges = data.edge_index.size(1)
            n_train = sum(1 for l in open(osp.join(_kg_raw, "train.txt")) if l.strip())
            n_valid = sum(1 for l in open(osp.join(_kg_raw, "valid.txt")) if l.strip())
            ...
            val_mask[n_train:n_train+n_valid] = True
            test_mask[n_train+n_valid:] = True
            return [{"train": train_mask, "val": val_mask, "test": test_mask}] * params['repeat']
        if not has_mask and not has_masks and has_idx:
            # Idx-style splits (goodreads, ml1m, ml1m_cls, protein_hs): convert to edge-level masks.
            n_rows = len(data.y) if hasattr(data, 'y') and data.y is not None else data.edge_index.size(1)
            train_mask = torch.zeros(n_rows, dtype=torch.bool); train_mask[data.train_idx] = True
            val_mask   = torch.zeros(n_rows, dtype=torch.bool); val_mask[data.val_idx]     = True
            test_mask  = torch.zeros(n_rows, dtype=torch.bool); test_mask[data.test_idx]   = True
            return [{"train": train_mask, "val": val_mask, "test": test_mask}] * params['repeat']
        if has_masks:
            assert len(data.train_masks[0]) == len(data.y)

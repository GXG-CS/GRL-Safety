"""GIT FT + step imbalance (node classification only).

Uses GIT's raw Encoder + simple linear head. Bypasses TaskModel complexity.
"""
...
def load_encoder_ckpt(encoder, ckpt_dir, ckpt_name="encoder.pt"):
    """Load encoder state from ckpt_dir/encoder.pt."""
    path = osp.join(ckpt_dir, ckpt_name)
    state = torch.load(path, map_location="cpu", weights_only=False)
    if isinstance(state, dict) and "model" in state and isinstance(state["model"], dict):
        state = state["model"]
    # Try loading — allow partial (ckpt might have extra keys)
...
def main():
    ...
    print(f"[GIT FT-IMB] device={device}, dataset={args.dataset}, rho={args.rho}, n_reps={args.n_reps}")
    ...
    print(f"[GIT FT-IMB] N={data.num_nodes}, d={input_dim}, C={num_classes}")
    ...
    for rep_idx in range(args.n_reps):
        ...
        model = train_ft(model, data.x, data.edge_index, y, imb_train_mask, val_mask,
        ...
        print(f"[IMB_RAW] method=GIT dataset={args.dataset} rho={args.rho} "
        ...
        print(f"[IMB_PER_CLASS] method=GIT dataset={args.dataset} rho={args.rho} "
    ...
    print(f"\n=== GIT Imbalance ({args.dataset}, rho={args.rho}) ===")
    print(f"  bAcc={bacc.mean():.2f} ± {bacc.std():.2f}  macroF1={f1.mean():.2f} ± {f1.std():.2f}")
    print(f"[IMB_AGG] method=GIT dataset={args.dataset} rho={args.rho} "

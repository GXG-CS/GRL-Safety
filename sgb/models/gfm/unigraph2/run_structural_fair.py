"""UniGraph2 FT + structural (degree-based) fairness."""
...
def build_model(num_features=768, num_hidden=768, num_layers=3, num_experts=8,
                num_selected_experts=2, feat_drop_rate=0.1, edge_mask_rate=0.1,
                gamma=2.0, lambda_spd=0.5):
    return UniGraph2(
        input_dims={"text": num_features}, hidden_dim=num_hidden,
        num_experts=num_experts, num_selected_experts=num_selected_experts,
        num_layers=num_layers, feat_drop_rate=feat_drop_rate,
        edge_mask_rate=edge_mask_rate, gamma=gamma, lambda_spd=lambda_spd,
    )
...
class FTModel(nn.Module):
    ...
    def forward(self, g, x):
        ...
        h = self.dropout(h); return self.head(h)
...
def load_dataset(name, device):
    ...
    if data.x is not None and data.x.dtype == torch.long and hasattr(data, "node_text_feat"):
        ...
        raise RuntimeError(f"no 768d for {name}")
    ...
    degree = torch.bincount(data.edge_index[0].long(), minlength=N).numpy()
    src, dst = data.edge_index[0], data.edge_index[1]
    g = dgl.graph((src, dst), num_nodes=N).remove_self_loop().add_self_loop()
    return g.to(device), feat.to(device), y.long().to(device), splits, degree
...
def train_ft(model, g, feat, y, tm, vm, device, max_epochs, patience, lr, wd):
    ...
    for _ in range(1, max_epochs + 1):
        ...
        if val_acc > best_val: best_val = val_acc; best_state = copy.deepcopy(model.state_dict()); no_improve = 0
        else:
            no_improve += 1
...
def main():
    ...
    print(f"[UG2 FT-STRUCT] N={N} d={input_size} C={num_classes} deg mean={degree.mean():.1f}")
    ...
    for split_idx, (tm, vm, tsm) in enumerate(splits):
        ...
        for rs in RUN_SEEDS:
            ...
            print(f"[STRUCT_RAW] method=UniGraph2_FT dataset={args.dataset} split={split_idx} seed={rs} "
    ...
    print(f'[STRUCT_AGG] method=UniGraph2_FT dataset={args.dataset} n_runs={len(results)} '

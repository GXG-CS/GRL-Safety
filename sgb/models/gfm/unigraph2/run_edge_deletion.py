"""UniGraph2 FT + edge-deletion corruption evaluation.

Parallel to sgb/models/ssl/graphmae/run_edge_deletion.py, node classification only.
Loads the joint-pretrained UniGraph2 encoder, attaches a linear head, fine-tunes
end-to-end on the clean graph, then evaluates under 5 edge-deletion severity levels.
"""
...
# Reuse FTModel + build_model + SEVERITIES live only in feature_noise; we
# redeclare here to keep this file self-contained and importable standalone.
...
def load_dataset(name, device):
    """Return feat, y, base_edge_index, num_nodes, splits — graph built per
    severity since edges change."""
...
def make_dgl_graph(edge_index, num_nodes, device):
...
def main():
    ...
    print(f"[UG2 FT-ED] Using {device}")
    ...
    clean_graph = make_dgl_graph(base_ei, num_nodes, device)
    print(f"[UG2 FT-ED] {args.dataset}, N={num_nodes}, E={base_ei.size(1)}, C={num_classes}")
    ...
    for split_idx, split in enumerate(splits):
        ...
        clean_acc, clean_f1 = train_ft(
            model, clean_graph, feat, y, train_mask, val_mask, test_mask,
        ...
        print(
            f"[ED_RAW] method=UniGraph2_FT dataset={args.dataset} "
        ...
        for sev_idx, p in SEVERITIES:
            ...
            noisy_graph = make_dgl_graph(dropped_ei, num_nodes, device)
            with torch.no_grad():
    ...
    print(f"\n=== UniGraph2 FT Edge Deletion Results ===")
    ...
    print(
        f"[ED_AGG] method=UniGraph2_FT dataset={args.dataset} "
...
def _run_link_ft_ed(device, args):
    ...
    print(f"[UG2 FT-ED Link] {args.dataset}, N={num_nodes}, E={graph.edge_index.size(1)}")
    ...
    for split_idx in range(5):
        ...
        def _eval_split(data_split):
            ...
            f1val = f1_score(y_np.astype(int), pred_bin, average='macro', zero_division=0) * 100.0
            return auc, f1val
        ...
        print(f"[ED_RAW] method=UniGraph2_FT dataset={args.dataset} "
              f"split_idx={split_idx} seed={split_idx} sev=0 p=0.0 "
              f"test_auc={clean_auc:.4f} test_f1={clean_f1:.4f}")

        clean_ei = test_data.edge_index.clone()
        for sev_idx, p in SEVERITIES:
            test_data.edge_index = apply_edge_drop(clean_ei, test_data.num_nodes, p)
    ...
    print(f"[ED_AGG] method=UniGraph2_FT dataset={args.dataset} "
          f"clean=\"{agg.get(0,'')}\" sev1=\"{agg.get(1,'')}\" sev2=\"{agg.get(2,'')}\" "
          f"sev3=\"{agg.get(3,'')}\" sev4=\"{agg.get(4,'')}\" sev5=\"{agg.get(5,'')}\" "
          f"clean_f1=\"{agg_f1.get(0,'')}\" sev1_f1=\"{agg_f1.get(1,'')}\" sev2_f1=\"{agg_f1.get(2,'')}\" "
          f"sev3_f1=\"{agg_f1.get(3,'')}\" sev4_f1=\"{agg_f1.get(4,'')}\" sev5_f1=\"{agg_f1.get(5,'')}\"")
...
def _run_graph_ft_ed(device, args):
    ...
    print(f"[UG2 FT-ED Graph] {args.dataset}: n_graphs={n_graphs}, num_tasks={num_tasks}")
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            for batch in train_loader:
                ...
                if is_multitask:
                    ...
                    mask_t = ~torch.isnan(targets)
                    loss = F.binary_cross_entropy_with_logits(logits[mask_t], targets[mask_t])
                else:

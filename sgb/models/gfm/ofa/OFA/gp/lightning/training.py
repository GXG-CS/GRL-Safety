...
def lightning_fit(
    ...
    if save_model:
        # Keep best-k=3 by val metric, plus every-epoch for resume safety.
        best_cb = ModelCheckpoint(
            ...
            save_last=True,
            save_top_k=3,
            filename=cktp_prefix + "best-{epoch}-{step}",
        )
        every_epoch_cb = ModelCheckpoint(
            ...
            save_top_k=-1,
            every_n_epochs=1,
            filename=cktp_prefix + "epoch-{epoch}",
        )
        callbacks.append(best_cb)
        callbacks.append(every_epoch_cb)

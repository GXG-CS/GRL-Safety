...
def main(params):
    ...
    # GFM-Safety: load pretrained encoder + llm_proj from a prior run without
    # forcing Lightning to resume optimizer/epoch state. Use this to initialize
    # from joint-pretrain weights when doing downstream single-task FT.
    if getattr(params, "pretrained_weights", None) is not None:
        print(f"[run_cdm] loading pretrained weights from {params.pretrained_weights}")
        payload = torch.load(params.pretrained_weights, map_location="cpu", weights_only=False)
        enc_missing = model.model.load_state_dict(payload["encoder"], strict=False)
        proj_missing = model.llm_proj.load_state_dict(payload["llm_proj"], strict=False)
        print(f"[run_cdm] encoder missing/unexpected={enc_missing}")
        print(f"[run_cdm] llm_proj missing/unexpected={proj_missing}")
    ...
    # GFM-Safety extension: always save checkpoints, enable resume.
    ckpt_dir = getattr(params, "ckpt_dir", None)
    resume_ckpt = getattr(params, "resume_ckpt", None)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_dir", type=str, default=None,
                        help="Directory to save Lightning checkpoints (default: auto)")
    parser.add_argument("--resume_ckpt", type=str, default=None,
                        help="Path to a .ckpt file to resume pretraining from.")
    parser.add_argument("--pretrained_weights", type=str, default=None,
                        help="Path to encoder_weights.pt (from load_pretrained.py). "
                             "Loads encoder + llm_proj only — fresh optimizer/epoch state.")
    ...
    cli_ckpt_dir = params.ckpt_dir
    cli_resume_ckpt = params.resume_ckpt
    cli_pretrained_weights = params.pretrained_weights
    configs = []
    configs.append(
        load_yaml(
            os.path.join(
    ...
    # Inject ckpt_dir / resume_ckpt from CLI so main() sees them via getattr.
    mod_params["ckpt_dir"] = cli_ckpt_dir
    mod_params["resume_ckpt"] = cli_resume_ckpt
    mod_params["pretrained_weights"] = cli_pretrained_weights

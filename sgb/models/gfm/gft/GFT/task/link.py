...
def _compute_link_auc_f1(pred, y, mask=None):
    """Compute AUC-ROC and macro-F1 for link prediction.

    Args:
        pred: [N, C] softmax probability tensor.
        y:    [N] integer class labels.
        mask: optional boolean or index mask.

    Returns:
        (auc, f1) as Python floats (0-100 scale).
    """
    if mask is not None:
    ...
    pred_np = pred_masked.detach().cpu().numpy()
    y_np = y_masked.detach().cpu().numpy()
    y_pred_classes = pred_np.argmax(axis=1)

    num_classes = pred_np.shape[1]

    # F1 (macro)
    f1 = f1_score(y_np, y_pred_classes, average='macro') * 100

    # AUC-ROC: per-class OvR, skip classes with no support in y_true.
    import numpy as _np
    if num_classes == 2:
        try:
            auc = roc_auc_score(y_np, pred_np[:, 1]) * 100
        except ValueError:
            auc = 0.0
    else:
        auc_list = []
        for c in range(num_classes):
            ...
            if 0 < npos < len(y_np):
                try:
                    auc_list.append(roc_auc_score(pos.astype(int), pred_np[:, c]))
                except ValueError:
                    pass
        auc = float(_np.mean(auc_list) * 100) if auc_list else 0.0
...
def ft_link(model, dataset, loader, optimizer, split, labels, params, scheduler=None, **kwargs):
    ...
    if not mini_batch:
        ...
        if use_lin_clf:
            # Subsample to keep VQ memory bounded (e.g. ml1m has ~2M train edges).
            if edge_z_train.size(0) > 200_000:
                idx = torch.randperm(edge_z_train.size(0), device=edge_z_train.device)[:200_000]
                act_loss = model.compute_activation_loss(edge_z_train[idx], y_train[idx]) * params["lambda_act"]
            else:
...
def eval_link(model, dataset, loader, split, labels, params, **kwargs):
    ...
    if not mini_batch:
        ...
        if setting == "standard":
            ...
            test_auc, test_f1 = _compute_link_auc_f1(pred, y, test_mask)

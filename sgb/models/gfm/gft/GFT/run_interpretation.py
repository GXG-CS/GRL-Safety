"""V2: subgraph-ablation interpretation for GFT (PyG-style with edge_attr)."""
import argparse
import copy
...
from sgb.models.gfm.gft.GFT.run_fairness import (
    FTModel, _get_splits, train_ft)
...
def main():
    ...
    print(f"[GFT V2-INTERP] device={device}, dataset={args.dataset}")
    ...
    edge_text_proto = data.edge_text_feat  # (1, d_edge) or similar
    edge_attr_full = edge_text_proto.expand(base_ei.size(1), -1).contiguous()
    print(f"[GFT V2-INTERP] N={N} d={input_dim} C={num_classes} E={base_ei.size(1)}")
    ...
    for rs in RUN_SEEDS[:n_seeds]:
        ...
        model = train_ft(model, x_clean, base_ei, edge_attr_full, y, tm, vm,
                         args.lr, args.wd, args.max_epochs, args.patience)
        model.eval()
        with torch.no_grad():
            pred = model(x_clean, base_ei, edge_attr_full).argmax(-1)
        acc = accuracy_score(y[tsm].cpu().numpy(), pred[tsm].cpu().numpy()) * 100.0
        print(f"[GFT V2-INTERP] seed={rs} clean_acc={acc:.2f}")

        def forward_fn(edge_index_mod, x_arg, _model=model, _proto=edge_text_proto):
            ea = _proto.expand(edge_index_mod.size(1), -1).contiguous()
            return _model(x_arg, edge_index_mod, ea)
        ...
        for explainer in explainers:
            ...
            for tk in topk_list:
                ...
                print(f"[INTERP_NODE_V2_RAW] method=GFT dataset={args.dataset} "
    ...
    for (explainer, tk), d in agg.items():
        ...
        print(f"[INTERP_NODE_V2_AGG] method=GFT dataset={args.dataset} "

"""GFT FT + demographic fairness on tolokers (encoder-only variant).
...
Uses encoder + linear head (bypasses VQ prototype classifier) so that
softmax probabilities are well-defined for AUC-based fairness metrics.
"""
...
def train_ft(model, x, edge_index, edge_attr, y, tm, vm, lr, wd, max_epochs, patience):
    ...
    for ep in range(1, max_epochs + 1):
        ...
        model.eval()
        with torch.no_grad():
            probs = torch.softmax(model(x, edge_index, edge_attr), dim=-1)[:, 1].cpu().numpy()
            yv = y[vm].cpu().numpy()
...
def main():
    ...
    print(f"[GFT FT-FAIR] device={device}, dataset={args.dataset}")
    ...
    # GFT encoder requires per-edge edge_attr.
    ...
    sens_np = sens.numpy()
    print(f"[GFT FT-FAIR] N={N}, d={input_dim}, C={num_classes}")
    print(f"[GFT FT-FAIR] sens: {sens_meta}")
    ...
    for split_idx, (tm, vm, tsm) in enumerate(splits):
        ...
        for rs in RUN_SEEDS:
            ...
            print(f"[FAIR_RAW] method=GFT dataset={args.dataset} split={split_idx} seed={rs} "
    ...
    print(f'[FAIR_AGG] method=GFT dataset={args.dataset} n_runs={len(results)} '
